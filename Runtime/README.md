...
`nicyruntime` is designed to be a flexible and high-performance runtime for Luau scripts. It's built in Rust and can be loaded dynamically by a host application. It provides a sandboxed environment for Luau scripts with a custom `require` implementation that supports caching, file fingerprinting, and aliasing.

To enable Luau CodeGen/JIT for a specific file, add `--!native` on the first line of that file (entry or required module).
On Android/Termux builds, CodeGen/JIT is disabled for runtime stability, so `--!native` is ignored and `runtime.hasJIT(...)` returns `false`.
...
- `runtime.version`: The version of the `nicyruntime` library.
- `runtime.hasJIT(path?: string)`: Returns `true` if JIT/CodeGen is active for the given module path. Without argument, it checks the current file. On Android/Termux this always returns `false`.
...
* **`const char* nicy_version()`** **Version Info:** Returns a pointer to a null-terminated string containing the Nicy Runtime version (e.g., "Nicy Runtime 1.0.0-alpha").

* **`const char* nicy_luau_version()`** **Luau Version:** Returns a pointer to a null-terminated string containing the Luau release version from the linked engine (e.g., `0.709`).
